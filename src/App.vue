...
import ClaudeConfig from './views/ClaudeConfig.vue'
import ZcodeConfig from './views/ZcodeConfig.vue'
import TokenStats from './views/TokenStats.vue'
import ToastContainer from './components/ToastContainer.vue'

type Tab = 'balance' | 'zcode' | 'claude' | 'stats' | 'settings'

const activeTab = ref<Tab>('balance')
const transitionName = ref('fade')

const tabOrder: Tab[] = ['balance', 'zcode', 'claude', 'stats', 'settings']
...
// macOS Overlay 标题栏的交通灯按钮占据窗口左上角，品牌区需要让位
...
<template>
  ...
  <aside class="sidebar" @mousedown="handleDrag">
    <div :class="['sidebar-brand', { 'sidebar-brand-mac': isMac }]">
    ...
    <nav class="sidebar-nav">
      ...
      <button
        :class="['nav-item', { active: activeTab === 'zcode' }]"
        @click="switchTab('zcode')"
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  ...
  <main class="main-area" @mousedown="handleDrag">
    <div class="page-content">
      ...
      <Transition :name="transitionName" mode="out-in">
        ...
        <ZcodeConfig v-else-if="activeTab === 'zcode'" key="zcode" :zhipu-api-key="apiKey" />
        <ClaudeConfig v-else-if="activeTab === 'claude'" key="claude" :zhipu-api-key="apiKey" />
        <TokenStats v-else-if="activeTab === 'stats'" key="stats" />
